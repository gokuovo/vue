<template>
  <div>
    ...
    <div class="container" v-if="!showEdit">
      <div class="header">
        <div class="title">worklog列表</div>
      </div>
      <!-- 表格 -->
      <el-table :data="worklog" v-loading="loading">
  ...
  export default {
    ...
    setup() {
      ...
      const getWorklog = async () => {
        try {
          loading.value = true
          worklog.value = await get("/SaltNews/getWorklog")
          loading.value = false
        } catch (error) {
          loading.value = false
          if (error.code === 10020) {
      ...
      const handleDelete = id => {
        ElMessageBox.confirm('此操作将永久删除该友商, 是否继续?', '提示', {
          ...
          const res = await post("/SaltNews/deleteWorklog?id="+id)
