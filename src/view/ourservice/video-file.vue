<template>
  <div class="container">
    <div class="title">
      <span>上传视频</span> <span class="back" @click="back"> <i class="iconfont icon-fanhui"></i> 返回 </span>
    </div>
    ...
    <div class="lin-wrap">
      <el-form label-width="220px">
        <el-form-item label="视频上传">
          <input class="fileUploaderClass" type='file' name="uploadBtn" ref="uploadBtn" @change="upLoadFile(video.id)"/>
        </el-form-item>
      </el-form>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    setup(props, context) {
      ...
      const video = reactive({ id: '',videoNameEn: '', videoNameChi: '', videoNameJap: '', videoNameSpa: '',
        videoUrl: ''})
      ...
      const getVideo = async () => {
        loading.value = true
        const res = await get('/SaltOurService/getVideoOne?id='+props.editVideoId)
        listAssign(video, res)
        loading.value = false
      }
