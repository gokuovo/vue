  ...
  <div class="container">
    ...
    <div class="lin-wrap">
      <el-form label-width="220px">
        <el-form-item label="当前配图" v-if="news.url">
        ...
        <el-form-item label="配图上传">
          <input class="fileUploaderClass" type='file' name="uploadBtn" ref="uploadBtn" @change="upLoadFile(news.id)"/>
        </el-form-item>
      </el-form>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    setup(props, context) {
      ...
      const getNews = async () => {
        loading.value = true
        const res = await get('/SaltNews/getNewsOne?id='+props.newsId)
        listAssign(news, res)
        loading.value = false
      }
