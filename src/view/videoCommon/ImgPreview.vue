  ...
  <transition name="el-fade-in-linear el-fade-in">
    ...
    >
      ...
      <div class="tip-wrapper" v-if="imgPreviewVisible">
        <div class="name" :title="activeFileName">{{ activeFileName }}</div>
        ...
        <div class="tool-wrapper">
          ...
          <el-tooltip effect="dark" placement="bottom">
            ...
            <div class="item text-wrapper" style="text-align: left;width: 25%">
...
<script>
  export default {
    ...
    mounted(){
      this.itemRefs = document.getElementsByClassName("img-large");
    },
    computed: {
      // 图片查看组件是否显示
      imgPreviewVisible() {
    ...
    watch: {
      ...
      activeIndex(newValue) {
        this.rotate = 0
        this.$nextTick(() => {
          if (this.itemRefs[newValue].style.zoom) {
            this.imgZoom = Number(this.itemRefs[newValue].style.zoom.split('%')[0])
          } else {
            this.itemRefs[newValue].style.zoom = '40%'
            this.imgZoom = 40
          }
        })
      }
    ...
    methods: {
      ...
      closeImgReview() {
        ...
        this.rotate = 0
        this.itemRefs[this.activeIndex].style.transform = `rotate(${this.rotate}deg)`
      },
      /**
      ...
      changeZoom(value) {
        if (this.itemRefs) {
          this.itemRefs[this.activeIndex].style.transform = `scale(${value/100})` //  实时设置图片缩放比例
        }
      },
      /**
       * 缩放图片
       */
      rollImg() {
        let zoom = parseInt(this.itemRefs[this.activeIndex].style.zoom) || 100
        zoom += event.wheelDelta / 12
        if (zoom >= this.imgZoomMin && zoom < this.imgZoomMax) {
          this.imgZoom = zoom
          this.itemRefs[this.activeIndex].style.transform = `scale(${zoom/100})`
        }
        return false
      },
      /**
